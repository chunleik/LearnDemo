...
class FindTest(unittest.TestCase):
    ...
    def test_01_SearchAll(self):
        ...
        self.failUnless(list(map(filename,res)) == ['file1.txt', 'file2.py'], 'wrong results')
    ...
    def test_02_SearchFileName(self):
        ...
        self.failUnless(list(map(filename, res)) == ['file2.py'], 'Python file search incorrect')
    
    def test_03_SearchByContent(self):
        """ 3:Test searching by content."""
        res = find.find(start="_test", content="first")
        self.failUnless(list(map(filename,res)) == ['file1.txt'], "didn't find file2.py")
        res = find.find(where="py$", start="_test", content="line")
        self.failUnless(list(map(filename,res)) == ['file2.py'], "didn't find file2.py")
    ...
    def test_04_SearchByExtension(self):
        ...
        self.failUnless(list(map(filename,res)) == ['file1.txt'], "didn't find file1.txt")

    def test_05_SearchByLogic(self):
        """ 5:Test searching using a logic combination callback."""
        res = find.find(start="_test", logic=lambda x: (x['size'] < 75))
        self.failUnless(list(map(filename, res)) == ['file1.txt'], "failed to find by size")
