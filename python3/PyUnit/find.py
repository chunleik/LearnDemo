...
def find_file(context, dir, files):
    for file in files:
        ...
        # Do content filtration last, to avoid it as much as possbile
        if context['content']:
            f = open(path, 'r')
            match = 0
        ...
        # Build the return value for any files that pased the filtration tests.
        file_return = (path, file, ext,size)
        context['return'].append(file_return)


if __name__ == '__main__':
    result = find(r"py$", content='find')
    print(result)